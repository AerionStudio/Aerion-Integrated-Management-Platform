...
export default {
  ...
  methods: {
    ...
    GetUserTime() {
      axios
        .get(
          "https://server.skydreamclub.cn/GetUserData.php?callsign=" +
        ...
        .then((response) => {
          ...
          this.Hourlywages_atc= this.getatch(atcTimeInSeconds);
      ...
      axios
        .get(
          "https://server.skydreamclub.cn/GetUserFlightAtcDataByID.php?callsign=" +
            sessionStorage.getItem("user_num")
        )
        .then((response) => {
          const data = response.data;
    ...
    getatch(time){
      if (time >= 360000 && time < 720000) {
        return 60;
      } else if (time >= 720000 && time < 1080000) {
        return 80;
      } else if (time >= 1080000 && time < 1440000) {
        return 100;
      } else if (time >= 1440000 && time < 1800000) {
        return 120;
      } else if (time >= 1800000 && time < 2160000) {
        return 140;
      } else if (time >= 2160000) {
        return 160;
      } else {
        return 40;
      }
    ...
    async getflightgradeimage(time) {
      ...
      try {
        ...
        const response = await axios.post(
          `https://server.skydreamclub.cn/GetUserFlightImage.php`,
          requestData
        );
    ...
    async getflightatcimage() {
      try {
        ...
        const response = await axios.post(
          `https://server.skydreamclub.cn/GetUserATCImage.php`,
          requestData
        );
    ...
    async getIntegral() {
      try {
        let time=this.flightTimeInSeconds;
        console.log(time);
  ...
  <transition name="el-fade-in-linear">
    <div v-show="main">
      ...
      <el-row :gutter="20">
        <el-col :span="8">
          ...
          <el-card
            style="
              text-align: center;
              background: linear-gradient(90deg, #86d0d1, #009eff, #1134ff);
              display: flex;
          ...
          >
            ...
            <h1 style="font-size: 25px; color: #ffffff">
              {{ usernum }} | {{ usergradetext }}
            </h1>
            <h1 style="font-size: 20px; color: #ffffff">飞行员时薪 {{Hourlywages}}
            </h1>
            <h1 style="font-size: 20px; color: #ffffff" v-if="usergrade>1">管制员时薪 {{Hourlywages_atc}}元/小时
            </h1>
            <br />
            <div
              style="
          ...
          <el-card
            class="rounded-card"
            style="background: #1f91dc; color: #ffffff; font-size: 30px"
          >
          <span class="num">{{integral}} 元</span
            ><br />
          ...
          <el-card
            class="rounded-card"
            style="background: #009eff; color: #ffffff; font-size: 30px"
          >
          <span class="num">{{ flighttime }}</span
            ><br />
          ...
          <el-card
            class="rounded-card"
            style="background: #1cbce8; color: #ffffff; font-size: 30px"
          >
          <span class="num">{{ atctime }}</span
            ><br />
        ...
        <el-col :span="16">
          ...
          <el-card
            ...
            <br />
            <el-table :data="atcdata" border style="width: 100%; height: 240px">
      ...
      <el-card>
        ...
        <br />
        <el-button type="primary" style="width: 100%" @click="loginout"
          >登出</el-button
        >
      </el-card>
    </div>
