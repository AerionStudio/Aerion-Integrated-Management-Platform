...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    for ($i = 0; $i < count($sidstardata['Airport']['Approach']); $i++) {
       if ($sidstardata['Airport']['Approach'][$i]['Name']==$rwy){
           for ($j = 0; $j < count($sidstardata['Airport']['Approach'][$i]['App_Transition']); $j++) {
               $output[$j] = ['value' => $sidstardata['Airport']['Approach'][$i]['App_Transition'][$j]['Name'], 'label' => $sidstardata['Airport']['Approach'][$i]['App_Transition'][$j]['Name']
               ];
